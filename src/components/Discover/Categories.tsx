...
function matchesDateFilter(eventDateStr: string, dateFilter?: string): boolean {
      ...
      switch (dateFilter) {
            ...
            case "This Month": {
                  return eventDay.getFullYear() === today.getFullYear() && eventDay.getMonth() === today.getMonth();
            }
            default:
                  return true;
      }
...
export default function Categories({ search, city, price, date }: CategoriesProps) {
                        ...
                        const eventsWithPrices: EventWithPrice[] = await Promise.all(
                              result.data.items.map(async (event) => {
                                    try {
                                          ...
                                          const minPrice =
                                                detailData.data.ticketTypes.length > 0 ? Math.min(...detailData.data.ticketTypes.map((t) => t.price)) : 0;
      ...
      const filteredEvents = events.filter((event) => {
            ...
            const matchesCity = !city || event.venue.toLowerCase().includes(city.toLowerCase());
      ...
      return (
            <div className="px-6 md:px-16 py-16 bg-neutral-950">
                  ...
                  <p className="text-neutral-400 text-sm mb-6">{loading ? "Loading events..." : `${filteredEvents.length} events found`}</p>
                  ...
                  {!loading && paginatedEvents.length === 0 && <p className="text-neutral-400 text-center py-16">No events match your filters.</p>}
                        ...
                        <div className="flex items-center justify-center gap-2 mt-10">
                              ...
                              {Array.from({ length: totalPages }, (_, idx) => idx + 1).map((p) => (
                                    <Button
                                          ...
                                          onClick={() => setPage(p)}
                                          className={
                                                p === page ? "bg-purple-500 hover:bg-purple-600 text-white" : "bg-neutral-900 hover:bg-neutral-800 text-white"
                                          }
                                    >
                                          {p}
                                    </Button>
