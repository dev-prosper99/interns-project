...
async function fetchEventMinPrice(
  eventId: string,
  signal: AbortSignal,
): Promise<number | null> {
  try {
  ...
  useEffect(() => {
    ...
    async function fetchEvents() {
      ...
      try {
        ...
        const withPricePlaceholders: EventWithPrice[] = json.data.items.map(
          (item) => ({
            ...item,
            minPrice: null,
            priceLoading: true,
        ...
        withPricePlaceholders.forEach(async (item) => {
          ...
          setEvents((prev) =>
            prev.map((e) =>
              e.id === item.id ? { ...e, minPrice, priceLoading: false } : e,
            ),
          );
        });
      } catch (err) {
  ...
  return (
    <div className="pl-6 md:pl-30 py-16 bg-neutral-950">
      <div className="max-w-6xl mx-auto">
        ...
        {isLoading && (
          <p className="text-neutral-400 text-sm">Loading events...</p>
        )}

        {error && !isLoading && (
          <p className="text-red-400 text-sm">Couldn't load events: {error}</p>
        )}

        {!isLoading && !error && events.length === 0 && (
          <p className="text-neutral-400 text-sm">No upcoming events yet.</p>
        )}
              ...
              return (
                ...
                >
                  <EventCard
                    ...
                    ticketPrice={
                      event.priceLoading ? "" : (event.minPrice ?? 0)
                    }
                  />
                </div>
              );
