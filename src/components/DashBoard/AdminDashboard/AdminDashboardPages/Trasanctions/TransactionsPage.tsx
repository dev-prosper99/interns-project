...
import { type Transaction, type TransactionStatus, SAMPLE_TRANSACTIONS, formatNaira } from "./transactions.data";
...
export default function TransactionsPage({ transactions = SAMPLE_TRANSACTIONS, onRefund, onExport, pageSize = 8 }: TransactionsPageProps) {
      ...
      const revenueTotal = localTransactions.filter((t) => t.status === "Completed").reduce((sum, t) => sum + t.amount, 0);
      const refundedTotal = localTransactions.filter((t) => t.status === "Refunded").reduce((sum, t) => sum + t.amount, 0);
      ...
      async function confirmRefund() {
            ...
            try {
                  ...
                  setLocalTransactions((prev) => prev.map((t) => (t.id === refundTxn.id ? { ...t, status: "Refunded" } : t)));
                  setRefundTxn(null);
            } finally {
                  ...
                  <main className="min-w-0 flex-1">
                        ...
                        <div className="px-6 py-6 text-neutral-50">
                              ...
                              <Pagination items={filtered} initialPageSize={pageSize}>
                                    {(paginated) => <TransactionsTable transactions={paginated} onViewDetails={setDetailsTxn} />}
                              </Pagination>

                              {detailsTxn && <TransactionDetailsModal transaction={detailsTxn} onClose={() => setDetailsTxn(null)} onRefund={requestRefund} />}

                              {refundTxn && <RefundConfirmModal isSubmitting={isRefunding} onCancel={() => setRefundTxn(null)} onConfirm={confirmRefund} />}
                        </div>
                  </main>
            </div>
      );
