...
export default function EventDetails({ event, onClose, onBack, onEdit }: EventDetailsProps) {
  return (
    <div
    ...
    >
      ...
      <div className="flex flex-col gap-6 overflow-y-auto scrollbar-thin scrollbar-thumb-neutral-700 scrollbar-track-transparent">
        <Pill label={event.status} color={statusColors[event.status] ?? "#6B7280"} />
        <EventDetailGrid event={event} />
        <TicketTiersSection ticketTiers={event.ticketTiers} />
        <PromoCodeBanner code={event.promoCode} discount={event.promoDiscount} />
