...
export default function EventsPage() {
  ...
  const [unpublishTarget, setUnpublishTarget] = useState<EventRowData | null>(
    null,
  );

  const [viewTargetId, setViewTargetId] = useState<string | number | null>(
    null,
  );
  ...
  const fetchEvents = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/api/Events/my-events/${userId}`,
        {
          method: "GET",
          headers: {
  ...
  useEffect(() => {
    ...
    const fetchEventDetails = async () => {
      ...
      try {
        ...
        const response = await fetch(
          `${API_BASE_URL}/api/Events/${viewTargetId}`,
          {
            method: "GET",
            headers: {
              Authorization: `Bearer ${token}`,
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/api/Events/${deleteTarget.id}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  return (
    <div className="flex min-h-screen bg-neutral-950 text-white">
      ...
      {(editTarget || isCreateModalOpen) && (
        <CreateEventModal
          isOpen={true}
          eventId={editTarget?.id}
          onClose={handleCreateOrEditModalClose}
        />
      )}
